function model  = train_model_oasis_diag_metric(data, similarity_mat, parms)
...
%%
  ...
  for i_iter = 1: num_steps;    
    ...
    if mod(i_iter, 5) == 1 
        distance2pos = (p_all - p_pos_all) * W * (p_all - p_pos_all)';
        distance2pos = diag(distance2pos);
        distance2neg = (p_all - p_neg_all) * W * (p_all - p_neg_all)';
        distance2neg = diag(distance2neg);
        l_W = max([zeros(length(distance2neg), 1), 1 - distance2neg + distance2pos], [], 2);
        loss_iter(i_iter) = sum(l_W);
        clear distance2pos distance2neg 
    end
    ...
    delta_pos = (p - p_pos);
    delta_neg = (p - p_neg);
    distance_pos = delta_pos*W*delta_pos';
    distance_neg = delta_neg*W*delta_neg';
    
    loss = 1 + distance_pos - distance_neg;
    
    if loss > 0 % Update W

        grad_W = delta_pos'*delta_pos - delta_neg'*delta_neg;
        loss_steps(i_iter) = 1;

        norm_grad_W = norm(grad_W, 'fro')^2;
